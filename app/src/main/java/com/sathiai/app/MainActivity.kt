                ...
                LazyColumn(
                    ...
                    result?.let { r ->
                        ...
                        item {
                            ...
                            Text(
                                "Auto-send is OFF. You choose what to copy/send."
                            )
                        }

                        items(r.replies) { reply ->
                            ReplyCard(reply, ctx)
                        }
...
private fun ReplyCard(
    ...
    Card(
        ...
        Column(
            ...
            if (showWhy && reply.why.isNotBlank()) {
                Text(reply.why)
            }
        }
    }
...
private fun SettingsScreen(
    ...
    var key by remember {
        mutableStateOf(GroqClient.getKey(ctx))
    }
    ...
    Column(
        ...
        Button(
            onClick = {
                GroqClient.saveKey(ctx, key.trim())
                saved = true
            },
        ...
        if (saved) {
            Text("✅ API key saved on this device.")
        }
        ...
        Text(
            "Privacy note: Groq recommends keeping API keys on a trusted backend. " +
                "This v0.2 direct-API mode is for your personal prototype only; " +
                "do not share an APK containing your key."
        )
    }
}
    ...
    suspend fun analyze(
        ...
        withContext(Dispatchers.IO) {
            ...
            require(apiKey.isNotBlank()) {
                "Open Settings and save your Groq API key first."
            }
...
Keep replies human, not robotic, not overly formal.
...
Give 5 replies with varied but useful styles.

Each reply should be sendable as-is.
...
""".trimIndent()
            ...
            if (code !in 200..299) {
                throw IllegalStateException(
                    "Groq API error $code: ${cleanError(raw)}"
                )
            }
            ...
            val boosterJson =
                data.optJSONArray("booster")
                    ?: org.json.JSONArray()

            val booster =
                buildList {

                    for (i in 0 until boosterJson.length()) {
                        add(
                            boosterJson.optString(i)
                        )
                    }
                }.filter {
                    it.isNotBlank()
